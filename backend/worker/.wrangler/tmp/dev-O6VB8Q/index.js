...
// .wrangler/tmp/bundle-ym8QOz/strip-cf-connecting-ip-header.js
...
app.get("/api/reunions/:year", async (c) => {
  const { data } = await db(c.env).from("reunions").select("*").eq("year", Number(c.req.param("year"))).single();
  if (!data)
...
app.get("/api/reunions/:year/media", async (c) => {
  ...
  if (!reunion)
    return c.json([]);
  const { data } = await db(c.env).from("media").select("*").eq("reunion_id", reunion.id).order("created_at", { ascending: false });
  return c.json(data ?? []);
});
app.get("/api/reunions/:year/video-count", async (c) => {
  ...
  const { count } = await db(c.env).from("media").select("*", { count: "exact", head: true }).eq("reunion_id", reunion.id).eq("type", "video");
...
app.get("/api/reunions/:year/program", async (c) => {
  ...
  if (!reunion)
    return c.json([]);
  const { data } = await db(c.env).from("program_events").select("*").eq("reunion_id", reunion.id).order("event_date", { ascending: true, nullsFirst: false }).order("start_time", { ascending: true, nullsFirst: true }).order("sort_order");
  return c.json(data ?? []);
});
app.get("/api/reunions/:year/links", async (c) => {
  ...
  if (!reunion)
    return c.json([]);
  const { data } = await db(c.env).from("links").select("*").eq("reunion_id", reunion.id).order("sort_order");
  return c.json(data ?? []);
});
app.get("/api/reunions/:year/contacts", async (c) => {
  ...
  if (!reunion)
    return c.json([]);
  const { data } = await db(c.env).from("contacts").select("*").eq("reunion_id", reunion.id).order("sort_order");
  return c.json(data ?? []);
});
...
app.delete("/admin/media/:id", async (c) => {
  ...
  if (data?.r2_key)
    await c.env.BUCKET.delete(data.r2_key);
  await db(c.env).from("media").delete().eq("id", c.req.param("id"));
  return new Response(null, { status: 204 });
});
async function getReunionByYear(env, year) {
  const { data } = await db(env).from("reunions").select("id").eq("year", Number(year)).single();
  return data;
}
__name(getReunionByYear, "getReunionByYear");
var src_default = app;
...
// .wrangler/tmp/bundle-ym8QOz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ym8QOz/middleware-loader.entry.ts
